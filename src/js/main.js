...
Swal.fire({
  ...
  Swal.mixin({
    ...
    if (result.value) {
      ...
      fetch('/embed/api/create?auth=' + data[0], { headers: { 'Content-Type': 'application/json' }, body, method: 'PUT' })
        ...
        .then((json) => {
          ...
          else Swal.fire({ title: '처리가 완료되었습니다', html: '디스코드 채팅방에 이 링크를 보내보세요<hr/><input class="form-control result" type="text" value="' + window.location.protocol + '//' + window.location.host + '/embed/' + json.eid + '">', icon: 'success', allowEscapeKey: false, allowOutsideClick: false, showConfirmButton: false })
        })
    }
  })
})
